package com.example.cardgame.ui.levelthree
...
class LevelThreeFragment : Fragment(), KodeinAware, CardAdapter.OnItemClickListener, ErrorListener {
    ...
    override fun onCreateView(
        ...
        viewmodel.getLevelById(3).observe(this, Observer {
            if (it == null) {
                viewmodel.insertSaveUpdateLevel(Level(3, 0, finished = false))
        ...
        viewmodel.getImages(4).observe(this, Observer {
        ...
        viewmodel.getListOfPhoto().observe(this, Observer {
            if (it.size == 8 && score.text != "0") {
                var s: Int = score.text.toString().toInt()
                viewmodel.insertSaveUpdateLevel(Level(3, s, finished = true))
            }
        })
        return view
    }
    ...
    override fun onItemClicked(photo: Photo) {
        viewmodel.imageClicked(photo, 3)
