package com.example.cardgame.ui.leveltwo
...
class LevelTwoFragment : Fragment(), KodeinAware, CardAdapter.OnItemClickListener, ErrorListener {
    ...
    override fun onCreateView(
        ...
        viewmodel.getLevelById(2).observe(this, Observer {
            if (it == null) {
                viewmodel.insertSaveUpdateLevel(Level(2, 0, finished = false))
        ...
        viewmodel.getImages(3).observe(this, Observer {
        ...
        viewmodel.getListOfPhoto().observe(this, Observer {
            if (it.size == 6 && score.text != "0") {
                var s: Int = score.text.toString().toInt()
                viewmodel.insertSaveUpdateLevel(Level(2, s, finished = true))
            }
        })
        return view
    }
    ...
    override fun onItemClicked(photo: Photo) {
        viewmodel.imageClicked(photo, 2)
